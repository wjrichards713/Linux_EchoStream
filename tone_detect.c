    ...
    for (int i = 0; i < MAX_TONE_DEFINITIONS; i++) {
        ...
        if (!global_tone_detection.tone_a_confirmed) {
            ...
            if (check_tone_definition(tone_def->tone_a_freq, tone_def, 0)) {
                ...
                // Debug: Show hit progress (debounced to avoid spam)
                static int last_hit_streak_log = 0;
                static int last_a_present_log = 0;
                if (a_hit_streak == HIT_REQUIRED && (current_time - last_hit_streak_log > 2000)) {
                    printf("[TONE] Tone A hit streak reached %d - starting confirmation process\n", HIT_REQUIRED);
                    last_hit_streak_log = current_time;
                }
                if (a_hit_streak >= HIT_REQUIRED && !a_present && (current_time - last_a_present_log > 2000)) {
                    printf("[TONE] Tone A present flag set - now tracking duration\n");
                    last_a_present_log = current_time;
                }
                ...
                if (!global_tone_detection.tone_a_tracking && a_present) {
                    ...
                    static int last_duration_tracking_log = 0;
                    if (current_time - last_duration_tracking_log > 2000) {
                        printf("[TONE] Tone A duration tracking started - need %d ms\n", tone_def->tone_a_length_ms);
                        last_duration_tracking_log = current_time;
                    }
        ...
        // Check for tone B (only if tone A was confirmed)
            ...
            if (check_tone_definition(tone_def->tone_b_freq, tone_def, 1)) {
                ...
                // Debug: Show hit progress (debounced to avoid spam)
                static int last_b_hit_streak_log = 0;
                static int last_b_present_log = 0;
                if (b_hit_streak == HIT_REQUIRED && (current_time - last_b_hit_streak_log > 2000)) {
                    printf("[TONE] Tone B hit streak reached %d - starting confirmation process\n", HIT_REQUIRED);
                    last_b_hit_streak_log = current_time;
                }
                if (b_hit_streak >= HIT_REQUIRED && !b_present && (current_time - last_b_present_log > 2000)) {
                    printf("[TONE] Tone B present flag set - now tracking duration\n");
                    last_b_present_log = current_time;
                }
                ...
                if (!global_tone_detection.tone_b_tracking && b_present) {
                    ...
                    static int last_b_duration_tracking_log = 0;
                    if (current_time - last_b_duration_tracking_log > 2000) {
                        printf("[TONE] Tone B duration tracking started - need %d ms\n", tone_def->tone_b_length_ms);
                        last_b_duration_tracking_log = current_time;
                    }
                    // Only log if we haven't been tracking recently (debounce)
...
static struct {
    int active;
    float detected_freq;  // Frequency of the SINGLE detected tone (deprecated - using tone_a_freq and tone_b_freq now)
    int is_tone_b;        // 1 if this is Tone B, 0 if Tone A (deprecated)
    float tone_a_freq;    // Frequency of Tone A
    float tone_b_freq;    // Frequency of Tone B
    int tone_a_length_ms; // Duration of Tone A in milliseconds
    int tone_b_length_ms; // Duration of Tone B in milliseconds
    float phase_a;        // Current phase for Tone A generation
    float phase_b;        // Current phase for Tone B generation
    int sample_rate;
    int samples_played;   // Total samples generated so far (for entire sequence)
    int tone_a_samples_played; // Samples played for Tone A
    int tone_b_samples_played; // Samples played for Tone B
    int tone_a_total_samples;  // Total samples needed for Tone A
    int tone_b_total_samples;  // Total samples needed for Tone B
    int current_tone;     // 0 = Tone A, 1 = Tone B, 2 = Done (both tones played)
    int total_samples;    // Total samples needed for record_length duration (deprecated)
    float phase;          // Current phase for tone generation (deprecated)
...
void trigger_tone_passthrough(struct tone_definition* confirmed_tone_def, int tone_type) {
    ...
    // Enable passthrough tone generation (will generate Tone A + Tone B pair)
    ...
    if (global_passthrough_tone.active) {
        // Already active - update frequencies for overlapping detection
...
int get_passthrough_tone_samples(float* output_buffer, int max_samples, int sample_rate) {
    ...
    if (!is_recording_active()) {
        ...
        if (global_passthrough_tone.active) {
            printf("[PASSTHROUGH] Recording timer expired - stopping tone generation (Tone A=%.1f Hz, Tone B=%.1f Hz, played for %d samples, ~%.1f seconds)\n",
                   global_passthrough_tone.tone_a_freq, global_passthrough_tone.tone_b_freq,
        ...
        global_passthrough_tone.current_tone = 0;
        return 0;
    }
    ...
    if (current_time - last_log_time > 1000) { // Log every 1 second
        int remaining_ms = get_recording_time_remaining_ms();
        printf("[DEBUG] Generating tone pair: Tone A=%.1f Hz, Tone B=%.1f Hz, Duration remaining=%d ms, Samples played=%d\n",
               global_passthrough_tone.tone_a_freq, global_passthrough_tone.tone_b_freq, remaining_ms, global_passthrough_tone.samples_played);
        last_log_time = current_time;
    }
    
    // Generate Tone A first, then Tone B sequentially (not mixed)
    int samples_generated = 0;
    
    for (int i = 0; i < samples_to_generate && samples_generated < samples_to_generate; i++) {
        if (global_passthrough_tone.current_tone == 0) {
            // Playing Tone A
            float tone_a_increment = 2.0f * M_PI * global_passthrough_tone.tone_a_freq / (float)global_passthrough_tone.sample_rate;
            output_buffer[samples_generated] = sin(global_passthrough_tone.phase_a) * 0.8f; // 80% volume
            ...
            global_passthrough_tone.tone_a_samples_played++;
            samples_generated++;
            
            // Check if Tone A is complete
            if (global_passthrough_tone.tone_a_samples_played >= global_passthrough_tone.tone_a_total_samples) {
                global_passthrough_tone.current_tone = 1; // Switch to Tone B
                global_passthrough_tone.phase_b = 0.0f; // Reset phase for Tone B
                printf("[PASSTHROUGH] Tone A complete (%d ms), starting Tone B (%.1f Hz for %d ms)\n",
                       global_passthrough_tone.tone_a_length_ms,
                       global_passthrough_tone.tone_b_freq,
                       global_passthrough_tone.tone_b_length_ms);
            }
        } else if (global_passthrough_tone.current_tone == 1) {
            // Playing Tone B
            float tone_b_increment = 2.0f * M_PI * global_passthrough_tone.tone_b_freq / (float)global_passthrough_tone.sample_rate;
            output_buffer[samples_generated] = sin(global_passthrough_tone.phase_b) * 0.8f; // 80% volume
            ...
            global_passthrough_tone.tone_b_samples_played++;
            samples_generated++;
            
            // Check if Tone B is complete
            if (global_passthrough_tone.tone_b_samples_played >= global_passthrough_tone.tone_b_total_samples) {
                global_passthrough_tone.current_tone = 2; // Both tones complete
                printf("[PASSTHROUGH] Tone B complete (%d ms), both tones played\n",
                       global_passthrough_tone.tone_b_length_ms);
                // Fill remaining buffer with silence
                for (int j = samples_generated; j < samples_to_generate; j++) {
            ...
            // Both tones complete - output silence
            output_buffer[samples_generated] = 0.0f;
            samples_generated++;
        }
    }
    
    global_passthrough_tone.samples_played += samples_generated;
