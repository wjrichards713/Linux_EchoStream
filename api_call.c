...
static int zero_key_warned[4] = {0};
static unsigned long jitter_drop_count[4] = {0};
static unsigned long decrypt_fail_count[4] = {0};
...
void* udp_listener_worker(void* arg) {
    ...
    while (!global_interrupted) {
        ...
        if (udp_debug_count++ % 1000 == 0 && udp_debug_enabled()) {
                                ...
                                if (samples > 0) {
                                    ...
                                    if (jitter->frame_count < JITTER_BUFFER_SIZE) {
                                        ...
                                        for (int j = 0; j < samples && j < SAMPLES_PER_FRAME; j++) {
                                            ...
                                            // Apply 20x gain boost for louder, clearer audio
                                            sample *= 20.0f;
