...
int init_gpio_pin(int pin) {
    ...
    // For pinctrl, we need to specify the gpiochip explicitly
    ...
    snprintf(cmd, sizeof(cmd), "pinctrl set %d ip pu -c %s", pinctrl_pin, gpiochip);
    printf("Executing: %s\n", cmd);
    result = system(cmd);
...
void cleanup_gpio(int pin) {
    ...
    snprintf(cmd, sizeof(cmd), "pinctrl set %d ip -c %s", pinctrl_pin, gpiochip);
